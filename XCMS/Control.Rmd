...
meso_stans <- read.csv("XCMS/meso_stans.csv")
...
write.csv(BMISed_feature_peaks, 
          file = paste0(pretty_folder, "BMISed_peaks.csv"), 
          row.names = FALSE)
...
std_only <- final_peaks %>% 
  group_by(feature) %>% 
  summarize(all_stan=all(station=="Std")) %>%
  filter(all_stan)

final_peaks <- final_peaks %>% 
  filter(!feature%in%found_stans$feature) %>%
  filter(!feature%in%std_only$feature)
...
# Direction data!
dir_diffs <- diffreport(final_peaks, pattern_a = "62|64|L1", pattern_b = "77|80|L2") %>%
  left_join(final_peaks %>% group_by(feature) %>%
              summarize(mzmed=median(mz), rtmed=median(rt)), by="feature")
dir_diffs$feature %>%
  lapply(function(feature_num){
    gp_df <- final_peaks %>%
      ...
      select(`Sample type`=spindir, everything())
    gp <- gp_df %>%
      ggplot() + geom_boxplot(aes(x=`Sample type`, y=BMISed_area, color=`Sample type`), varwidth = TRUE) +
      theme_bw() + theme(legend.position = "none") + ylab("BMIS-normalized peak area") +
