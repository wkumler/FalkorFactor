...
final_features <- read.csv(paste0(pretty_folder, "complete_features.csv"))
real_peaks <- read.csv(paste0(intermediate_folder, "real_peaks.csv"))
feature_formulas <- "feature_formulas.csv" %>%
  paste0(pretty_folder, .) %>%
  read.csv()
stan_annotations <- "stan_annotations.csv" %>%
  paste0(pretty_folder, .) %>%
  read.csv() %>%
...
falkor_stans <- read.csv("XCMS/falkor_stans.csv") %>% 
  ...
  add_row(compound_type="Custom", 
          ...
          ionization_form="[M+H-NH3]", charge=1, kegg_id=NA,
          polarity="pos", date_added=NA, mix="Mix1") %>%
  select(compound_name, formula, rt, mz, ionization_form)

formula_check <- stan_annotations %>%
  filter(!is.na(best_guess)) %>%
  left_join(feature_formulas, by=c(best_guess="feature")) %>%
  left_join(falkor_stans, by="compound_name") %>%
  select(feature="best_guess", compound_name, best_guess, ionization_form, formula_pred="formula.x", formula_known="formula.y", mz) %>%
  mutate(formula_pred=ifelse(ionization_form!="[M]", formula_pred, 
                             str_replace(formula_pred, pattern = "H\\d*", replacement = 
                                           paste0("H", as.numeric(str_extract(formula_pred, "(?<=H)\\d*"))+1)))) %>%
  rowwise() %>%
  mutate(top_pick=formula_known==unlist(strsplit(formula_pred, split = "; "))[1]) %>%
  as.data.frame()

formula_check %>%
  filter(!top_pick|is.na(top_pick)) %>%
  select(feature, compound_name, formula_pred, formula_known)
...
stan_annotations %>%
  ...
  filter(cruise=="Falkor") %>%
  ggplot() +
  geom_boxplot(aes(x=spindir, y=BMISed_area, color=spindir), varwidth = TRUE) +
  scale_color_manual(values = c(Anticyclone=rgb(204, 54, 17, maxColorValue = 255), 
                                Cyclone=rgb(6, 81, 164, maxColorValue = 255))) +
  facet_wrap(~compound_name, scales = "free_x") +
  coord_flip() +
  ggsave(filename = paste0(pretty_folder, "targeted_boxplots_spindir.pdf"),
       device = "pdf", width = 25, height = 25, units = "in")
...
stan_annotations %>%
  ...
  filter(cruise=="Falkor") %>%
  ggplot() +
  geom_boxplot(aes(x=time, y=BMISed_area, color=time), varwidth = TRUE) +
  scale_color_manual(values = c(`4am`="#fc8b64", `4pm`="#222a5c")) +
  facet_wrap(~compound_name, scales = "free_x") +
  coord_flip() +
  ggsave(filename = paste0(pretty_folder, "targeted_boxplots_time.pdf"),
       device = "pdf", width = 25, height = 25, units = "in")
```
