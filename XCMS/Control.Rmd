...
head(addiso_peaks)
addiso_peaks %>% filter(mz%between%pmppm(165.1325, 10)) %>% group_by(feature) %>% summarize(medmz=median(mz), medrt=median(rt), adduct_type=unique(adduct_type)) %>% as.data.frame() %>% head()
...
addiso_features <- read.csv(paste0(pretty_folder, "addiso_peaks.csv")) %>%
  group_by(feature) %>% summarize(medmz=median(mz), medrt=median(rt), adduct_type=unique(adduct_type)) %>% as.data.frame() %>% head()
...
library(git2r)
repo <- repository()
files_to_commit <- c("BMISed_peaks.csv", "addiso_peaks.csv", 
