...
will_findChromPeaks_plural <- function (object, param, ...) {
  .local <- function (object, param, BPPARAM = bpparam(), ...) {
    will_findChromPeaks_XChromatograms(object = object, param = param, BPPARAM = BPPARAM, ...)
  }
  .local(object, param, ...)
}

will_findChromPeaks_XChromatograms <- function (object, param, BPPARAM, ...) {
  startDate <- date()
  if (missing(BPPARAM)) 
    BPPARAM <- bpparam()
  object <- as(object, "XChromatograms")
  object@.Data <- matrix(bplapply(c(object@.Data), FUN = will_findChromPeaks, 
                                  param = param, BPPARAM = BPPARAM), ncol = ncol(object), 
  ...
  if (ph_len && processType(object@.processHistory[[ph_len]]) == 
    ...
    object@.processHistory <- object@.processHistory[seq_len(ph_len - 
                                                               1)]
  object <- xcms:::addProcessHistory(object, xcms:::XProcessHistory(param = param, 
                                                      date. = startDate, type. = xcms:::.PROCSTEP.PEAK.DETECTION, 
                                                      fileIndex = seq_len(ncol(object))))
  if (validObject(object)) 
    object
}

will_findChromPeaks <- function (object, param, ...) {
  res <- do.call("will_peaksWithCentWave", 
...
will_peaksWithCentWave <- function(int, rt, peakwidth=c(20,50), snthresh=10,
                                   prefilter=c(3,100), integrate=1, 
                                   fitgauss = F,noise=0, verboseColumns=F, 
  ...
  # i <- 3
    ...
    maxGaussOverlap <- 0.5 #Moved here from further up
