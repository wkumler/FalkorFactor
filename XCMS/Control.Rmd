...
safe_peaks <- lapply(not_addisos, function(safe_data){
  filter(raw_peaks, mz%between%pmppm(safe_data["mz"], 5)&
           rt%between%(safe_data["rt"]+c(-75, 75)))
}) %>% do.call(what = rbind)
raw_peaks <- anti_join(raw_peaks, safe_peaks)
...
all_stans <- read.csv("XCMS/falkor_stans.csv") %>% filter(polarity=="pos")
