...
## Unit test peaks picked

```{r}
real_features <- read.csv(paste0(pretty_folder, "complete_features.csv"))
addiso_features <- read.csv(paste0(pretty_folder, "addiso_features.csv"))
all_features <- bind_rows(real_features, addiso_features)
```
...
write.csv(stan_assignments, paste0(pretty_folder, "stan_annotations.csv"))
```
...
library(git2r)
repo <- repository()
files_to_commit <- c("final_peaks.csv", "addiso_features.csv", 
                     "complete_features.csv", "internal_stan_values.pdf",
                     "stan_v_stan.pdf", "stan_annotations.csv")
add(repo, paste0(pretty_folder, files_to_commit))
