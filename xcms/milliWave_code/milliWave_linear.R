...
ppm <- 5
peakwidth <- c(20, 80)
param <- CentWaveParam(peakwidth = peakwidth, ppm = ppm, 
...
object_mslevel <- filterMsLevel(
  selectFeatureData(object, fcol = c(MSnbase:::.MSnExpReqFvarLabels, "centroided")), 
  msLevel. = msLevel
)
...
method <- match.arg(method, c("centWave", "massifquant", "matchedFilter", 
                              "MSW", "centWaveWithPredIsoROIs", "milliWave"))
...
procDat <- date()
# SKIP THE BELOW AND CALL IT ONE AT A TIME
...
# SKIP THE BELOW AND DO IT DIRECTLY
...
# .milliWave ----
mz <- unlist(mzs, use.names = F)
int <- unlist(lapply(x, intensity), use.names = F)
valsPerSpect <- vals_per_spect
scantime <- rt
param <- as.list(param)
ppm = param$ppm
...
mzCenterFun <- param$mzCenterFun
integrate <- param$integrate
mzdiff <- param$mzdiff
fitgauss <- param$fitgauss
noise <- param$noise
verboseColumns <- param$verboseColumns
roiList <- param$roiList
firstBaselineCheck <- param$firstBaselineCheck
roiScales <- param$roiScales
...
if (length(roiList) == 0) {
  ...
  withRestarts(tryCatch({
    ...
    rm(splitF)
    tmp <- capture.output(roiList <<- .Call("findmzROI", 
                                            mz, int, scanindex, as.double(c(0, 0)), 
